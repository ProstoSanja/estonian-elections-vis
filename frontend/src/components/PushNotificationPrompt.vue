...
<template>
  <div class="flex flex-col self-stretch rounded-xl overflow-hidden p-0.5" :style="{
    ...
    <div v-if="specialOnboardingInProgress" class="flex flex-col gap-4 p-2 pl-4 pr-3">
      <span>iPhone kasutajatele on vaja teha paar lisa sammu Apple platformi piirangute tõttu. <span class="font-semibold">See võtab 30 sekundit.</span></span>

      <div class="flex flex-col gap-3 text-slate-200" v-if="specialOnboardingType === 'ios26'">
        <div class="flex flex-row gap-3 items-center">
          <span class="flex-shrink-0 w-6 h-6 rounded-full bg-slate-700 flex items-center justify-center text-sm font-semibold">1</span>
          <span class="">Vajutage <span class="bg-slate-700 rounded-md py-1 px-2 whitespace-nowrap">...</span> ekraani alumises paremas nurgas</span>
        </div>

        <div class="flex flex-row gap-3 items-center">
          <span class="flex-shrink-0 w-6 h-6 rounded-full bg-slate-700 flex items-center justify-center text-sm font-semibold">2</span>
          <span class="">Vajutage <span class="bg-slate-700 rounded-md py-1 px-2 whitespace-nowrap"><ArrowUpOnSquareIcon class="w-4 h-4 inline mt-[-4px]" /> Share</span> nuppu</span>
        </div>

        <div class="flex flex-row gap-3 items-center">
          <span class="flex-shrink-0 w-6 h-6 rounded-full bg-slate-700 flex items-center justify-center text-sm font-semibold">3</span>
          <span class="">Valige uuesti <span class="bg-slate-700 rounded-md py-1 px-2 whitespace-nowrap">...</span> ekraani alumises paremas nurgas</span>
        </div>

        <div class="flex flex-row gap-3 items-center">
          <span class="flex-shrink-0 w-6 h-6 rounded-full bg-slate-700 flex items-center justify-center text-sm font-semibold">4</span>
          <span class="">Vajutage <span class="bg-slate-700 rounded-md py-1 px-2 whitespace-nowrap"><FolderPlusIcon class="w-4 h-4 inline mt-[-4px]" /> Add to Home Screen</span> nuppu</span>
        </div>

        <div class="flex flex-row gap-3 items-center">
          <span class="flex-shrink-0 w-6 h-6 rounded-full bg-slate-700 flex items-center justify-center text-sm font-semibold"><CheckIcon class="w-4 h-4 inline" /></span>
      ...
      <div class="flex flex-col gap-3 text-slate-200" v-else-if="specialOnboardingType === 'ios'">
        <div class="flex flex-row gap-3 items-center">
          <span class="flex-shrink-0 w-6 h-6 rounded-full bg-slate-700 flex items-center justify-center text-sm font-semibold">1</span>
          <span class="">Vajutage <span class="bg-slate-700 rounded-md py-1 px-2 whitespace-nowrap"><ArrowUpOnSquareIcon class="w-4 h-4 inline mt-[-4px]" /> Share</span> nuppu aadressiriba juures</span>
        </div>

        <div class="flex flex-row gap-3 items-center">
          <span class="flex-shrink-0 w-6 h-6 rounded-full bg-slate-700 flex items-center justify-center text-sm font-semibold">4</span>
          <span class="">Kerige alla ja vajutage <span class="bg-slate-700 rounded-md py-1 px-2 whitespace-nowrap"><FolderPlusIcon class="w-4 h-4 inline mt-[-4px]" /> Add to Home Screen</span> nuppu</span>
        </div>
    ...
    <div v-else-if="pushNotifications.shouldPrompt || shouldShowSpecialOnboarding" class="flex flex-row items-center gap-4 p-2 pl-4 pr-3 cursor-pointer hover:brightness-125 transition"
    ...
    <!-- Success message -->
    <div v-else-if="pushNotifications.isSubscribed" class="flex items-center gap-4 text-green-200 p-2 pl-4 pr-3 hover:brightness-125 transition">
    ...
    <!-- Tutorial -->
    <div v-if="!specialOnboardingInProgress" class="flex flex-row items-center gap-4 self-stretch text-slate-500 hover:text-slate-400 transition-colors bg-slate-800 rounded-b-xl p-2 pl-4 pr-3">
